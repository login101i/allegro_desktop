import Image1 from "../assets/pictures/goodToSee1.png";
import Image2 from "../assets/pictures/goodToSee2.png";
import Image3 from "../assets/pictures/goodToSee3.png";
import Image4 from "../assets/pictures/goodToSee4.png";
import Image31 from "../assets/pictures/Image31.png";
import Image32 from "../assets/pictures/Image32.png";
import Image33 from "../assets/pictures/Image33.png";
import Image34 from "../assets/pictures/Image34.png";
import Image35 from "../assets/pictures/Image35.png";
import Image36 from "../assets/pictures/Image36.png";
import Image37 from "../assets/pictures/Image37.png";

// for sidebarIcons in secondsection
import LaptopChromebookOutlinedIcon from "@mui/icons-material/LaptopChromebookOutlined";
import BathroomOutlinedIcon from "@mui/icons-material/BathroomOutlined";
import AnchorIcon from "@mui/icons-material/Anchor";
import AnimationIcon from "@mui/icons-material/Animation";
import BabyChangingStationIcon from "@mui/icons-material/BabyChangingStation";
import BedIcon from "@mui/icons-material/Bed";
import BakeryDiningIcon from "@mui/icons-material/BakeryDining";
import AllInclusiveOutlinedIcon from "@mui/icons-material/AllInclusiveOutlined";
import BoltOutlinedIcon from "@mui/icons-material/BoltOutlined";
import BedOutlinedIcon from "@mui/icons-material/BedOutlined";
import ArchitectureOutlinedIcon from "@mui/icons-material/ArchitectureOutlined";
import ElectricCarOutlinedIcon from "@mui/icons-material/ElectricCarOutlined";

export const firstSectionImages = [
  { image: Image1, title: "Limitowana okazja!" },
  { image: Image2, title: "Alegro Smart" },
  { image: Image3, title: "Monety i wspólny Smart!" },
  { image: Image4, title: "Powrót do biura!" },
  { image: Image2, title: "Alegro Smart" },
  { image: Image3, title: "Monety i wspólny Smart!" },
  { image: Image4, title: "Powrót do biura!" },
  { image: Image2, title: "Alegro Smart" },
  { image: Image3, title: "Monety i wspólny Smart!" }
];
export const thirdSectionImages = [
  { id: 1, img: Image31, title: "Jesienna Pielęgnacja" },
  { id: 2, img: Image32, title: "Jesienna Pielęgnacja" },
  { id: 3, img: Image33, title: "Strefa Rehabilitacji" },
  { id: 4, img: Image34, title: "Telemedycyna [Vouchery] " },
  { id: 5, img: Image35, title: "Elektronika - top 100 " },
  { id: 6, img: Image36, title: "Okacje na smartfony " },
  { id: 7, img: Image37, title: "Telewizory i akcesoria " },
  { id: 8, img: Image31, title: "Bramy i ogrodzenia" },
  { id: 9, img: Image32, title: "Jesienna Pielęgnacja" },
  { id: 10, img: Image33, title: "Strefa Rehabilitacji" },
  { id: 11, img: Image34, title: "Telemedycyna [Vouchery] " },
  { id: 12, img: Image35, title: "Elektronika - top 100 " },
  { id: 13, img: Image36, title: "Okacje na smartfony " },
  { id: 14, img: Image37, title: "Telewizory i akcesoria " }
];

export const occasions = [
  {
    id: 1,
    img: "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d",
    discount: "12",
    oldPrice: "198",
    price: "129,78",
    SmartImage:
      "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d"
  },
  {
    id: 2,
    img: "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d",
    discount: "52",
    oldPrice: "498",
    price: "229,78",
    SmartImage:
      "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d"
  },
  {
    id: 3,
    img: "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d",
    discount: "52",
    oldPrice: "498",
    price: "129,78",
    SmartImage:
      "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d"
  },
  {
    id: 4,
    img: "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d",
    discount: "12",
    oldPrice: "198",
    price: "129,78",
    SmartImage:
      "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d"
  },
  {
    id: 5,
    img: "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d",
    discount: "12",
    oldPrice: "198",
    price: "129,78",
    SmartImage:
      "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d"
  },

  {
    id: 6,
    img: "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d",
    discount: "52",
    oldPrice: "498",
    price: "129,78",
    SmartImage:
      "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d"
  },
  {
    id: 7,
    img: "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d",
    discount: "12",
    oldPrice: "198",
    price: "129,78",
    SmartImage:
      "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d"
  },
  {
    id: 8,
    img: "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d",
    discount: "12",
    oldPrice: "198",
    price: "129,78",
    SmartImage:
      "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d"
  },
  {
    id: 9,
    img: "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d",
    discount: "12",
    oldPrice: "198",
    price: "129,78",
    SmartImage:
      "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d"
  },
  {
    id: 10,
    img: "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d",
    discount: "52",
    oldPrice: "498",
    price: "229,78",
    SmartImage:
      "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d"
  },
  {
    id: 11,

    img: "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d",
    discount: "52",
    oldPrice: "498",
    price: "129,78",
    SmartImage:
      "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d"
  },
  {
    id: 12,
    img: "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d",
    discount: "12",
    oldPrice: "198",
    price: "129,78",
    SmartImage:
      "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d"
  },
  {
    id: 13,
    img: "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d",
    discount: "52",
    oldPrice: "498",
    price: "229,78",
    SmartImage:
      "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d"
  }
];

export const tires = [
  {
    img: "https://a.allegroimg.com/s80/129ce8/62d1fc7d4daaacf91bfe6f94bf7d",
    discount: "12",
    oldPrice: "198",
    price: "129,78",
    SmartImage:
      "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d"
  },
  {
    img: "https://a.allegroimg.com/s80/129ce8/62d1fc7d4daaacf91bfe6f94bf7d",
    discount: "52",
    oldPrice: "498",
    price: "229,78",
    SmartImage:
      "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d"
  },
  {
    img: "https://a.allegroimg.com/s80/129ce8/62d1fc7d4daaacf91bfe6f94bf7d",
    discount: "52",
    oldPrice: "498",
    price: "129,78",
    SmartImage:
      "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d"
  },
  {
    img: "https://a.allegroimg.com/s80/129ce8/62d1fc7d4daaacf91bfe6f94bf7d",
    discount: "12",
    oldPrice: "198",
    price: "129,78",
    SmartImage:
      "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d"
  }
];

export const installments = [
  {
    img: "https://a.allegroimg.com/s512/033d6b/1f89fec048dfb35d7518ef72359a/Szafka-lazienkowa-z-lustrem-biala-polysk-80-cm",
    discount: "12",
    oldPrice: "198",
    price: "129,78",
    SmartImage:
      "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d"
  },
  {
    img: "https://a.allegroimg.com/s512/11ef1e/8c4b09ab4e1c963dca1c1c1a1733/Wiszaca-szafka-lazienkowa-pod-umywalke-60-cm-biala",
    discount: "52",
    oldPrice: "498",
    price: "229,78",
    SmartImage:
      "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d"
  },
  {
    img: "https://a.allegroimg.com/s512/11b81a/a7058b1b47768337c313e0de8a3f/Meble-lazienkowe-lustro-szafka-umywalka-2x-slupek",
    discount: "52",
    oldPrice: "498",
    price: "1129,78",
    SmartImage:
      "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d"
  },
  {
    img: "https://a.allegroimg.com/s512/11fe78/5b7b94e04dad80752be8032027a8/Meble-lazienkowe-lustro-szafka-umywalka-2x-slupek",
    discount: "12",
    oldPrice: "198",
    price: "1229,78",
    SmartImage:
      "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d"
  }
];
export const charity = [
  {
    img: "https://a.allegroimg.com/s512/1100af/c0cb8a26441ca6ea90c691c2c963/Plaszcz-kurtka-zimowa-Czaple-dziendobry-r-M",
    discount: "12",
    oldPrice: "198",
    price: "129,18",
    SmartImage:
      "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d"
  },
  {
    img: "https://a.allegroimg.com/s512/1100af/c0cb8a26441ca6ea90c691c2c963/Plaszcz-kurtka-zimowa-Czaple-dziendobry-r-M",
    discount: "52",
    oldPrice: "498",
    price: "233,78",
    SmartImage:
      "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d"
  },
  {
    img: "https://a.allegroimg.com/s512/1100af/c0cb8a26441ca6ea90c691c2c963/Plaszcz-kurtka-zimowa-Czaple-dziendobry-r-M",
    discount: "52",
    oldPrice: "798",
    price: "129,78",
    SmartImage:
      "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d"
  },
  {
    img: "https://a.allegroimg.com/s512/1100af/c0cb8a26441ca6ea90c691c2c963/Plaszcz-kurtka-zimowa-Czaple-dziendobry-r-M",
    discount: "12",
    oldPrice: "198",
    price: "429,78",
    SmartImage:
      "https://a.allegroimg.com/s80/12c474/6a60a34a494ca30a2c4a7859030d"
  }
];

export const fourthSectionData = [
  { id: 1, title: "Strefa okazji", array: occasions },
  {
    id: 2,
    title: "Opony zimowe Continental",
    array: tires
  },
  { id: 3, title: "Strefa Biletów", array: installments },
  {
    id: 4,
    title: "Strefa Rat Zero  ",
    array: charity
  },
  { id: 5, title: "Strefa ogłoszeń  ", array: occasions },
  {
    id: 6,
    title: "Strefa charytatywna  ",
    array: charity
  },
  {
    id: 7,
    title: "Strefa charytatywna  ",
    array: charity
  },
  { id: 8, title: "Strefa rat zero", array: charity }
];

export const firstSectionOptions = [
  "Kupuj teraz, płać później",
  "Karmy Dog Chow",
  "Z kuponami taniej",
  "Strefa projektanta",

  "Witaminy na jesień",
  "Zgarnij 30 Monet"
];

export const productCategories = [
  "Laptopy",
  "Telefony",
  "Perfumy",
  "Supermarket",
  "AGD",
  "Pupil"
];

export const sideBarCategoriesAdvantagesSecII = [
  { category: "Elektronika", icon: LaptopChromebookOutlinedIcon },
  { category: "Moda", icon: BathroomOutlinedIcon },
  { category: "Dom i Ogród", icon: AnchorIcon },
  { category: "Supermarket", icon: BabyChangingStationIcon },
  { category: "Dziecko", icon: AnimationIcon },
  { category: "Uroda", icon: BedIcon },
  { category: "Zdrowie", icon: LaptopChromebookOutlinedIcon },
  { category: "Kultura i Rozrywka", icon: BakeryDiningIcon },
  { category: "Sport i Turystyka", icon: LaptopChromebookOutlinedIcon },
  { category: "Motoryzacja", icon: ElectricCarOutlinedIcon },
  { category: "Nieruchomości", icon: LaptopChromebookOutlinedIcon },
  { category: "Kolekcje i sztuka", icon: AnimationIcon },
  { category: "Firma i usługi", icon: LaptopChromebookOutlinedIcon },
  { category: "eBilet", icon: BabyChangingStationIcon },
  { category: "Allegro Lokalnie", icon: BedIcon }
];

export const sideBarCategoriesBenefitsSecII = [
  { category: "Okazje", icon: BoltOutlinedIcon },
  { category: "Smart Okazje", icon: ArchitectureOutlinedIcon },
  { category: "Strefy zakupowe", icon: AllInclusiveOutlinedIcon },
  { category: "Kupony", icon: LaptopChromebookOutlinedIcon },
  { category: "Wszystko, czego szukasz", icon: BedOutlinedIcon },
  { category: "Hity z reklamy", icon: BedIcon },
  { category: "Premiery", icon: LaptopChromebookOutlinedIcon },
  { category: "Karty podarunkowe", icon: BakeryDiningIcon },
  { category: "Allegro Inspiruje", icon: BedIcon },
  { category: "Allegro Smart", icon: BathroomOutlinedIcon },
  { category: "Raty", icon: LaptopChromebookOutlinedIcon },
  { category: "Raty ZERO", icon: BakeryDiningIcon },
  { category: "Allegro Family", icon: BabyChangingStationIcon },
  { category: "Akademia Allegro", icon: LaptopChromebookOutlinedIcon },
  { category: "Zbieraj Monety", icon: BabyChangingStationIcon }
];

export const sliderItems = [
  {
    id: 1,
    img: "https://a.allegroimg.com/original/127cc8/f7bc8c1348f6b1c431c0548311f0"
  },
  {
    id: 2,
    img: "https://a.allegroimg.com/original/125ddf/b82aa9464d3cbb8abcb96274cbc4"
  },
  {
    id: 3,
    img: "https://a.allegroimg.com/original/12dcf8/404c61a34a29a9ff5e8d0c3f36cd"
  },
  {
    id: 4,
    img: "https://a.allegroimg.com/original/127b05/e99a5fff4544b66f1f0628220e72"
  },
  {
    id: 5,
    img: "https://a.allegroimg.com/original/12473a/a1cb531a44b1b6cb5caa99b9f35c"
  },
  {
    id: 6,
    img: "https://a.allegroimg.com/original/121c4a/c039318c41ab9e6d9867ff661b5e"
  },
  {
    id: 7,
    img: "https://a.allegroimg.com/original/121c4a/c039318c41ab9e6d9867ff661b5e"
  }
];
